...
class TestGmailClient(unittest.TestCase):
    ...
    def test_get_new_emails(self, mock_build, mock_get_credentials):
        ...
        self.assertEqual(emails[0]['thread_id'], 'thread1')
    ...
    @patch('src.bot.gmail_client.get_credentials')
    @patch('src.bot.gmail_client.build')
    def test_get_email_by_rfc822_msgid(self, mock_build, mock_get_credentials):
        mock_service = MagicMock()
        mock_build.return_value = mock_service
        ...
        client = GmailClient()
        email = client.get_email_by_rfc822_msgid('<abc@example.com>')

        self.assertEqual(email['id'], 'msg1')
        ...
        mock_service.users.return_value.messages.return_value.list.assert_called_once_with(
            userId='me', q='rfc822msgid:abc@example.com'
        )
